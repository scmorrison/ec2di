...
var Ec2di = function() {
  ...
  var getServiceRegions = function(service) {
    // Return all regions for a specific service (ec2, s3, rds, etc.)
  ...
  var filterRegions = function(regions, configRegions, configRegionsExclude) {
    // Filter out any excluded regions defined in the ini (configRegionsExclude)
        ...
        .then(function(exists) {
          if (exists) {
            ...
            fs.statAsync(settings.cachePathCache).then(function(stats) {
              // FileExpireTime is the file mod time + the cacheMaxAge as defined in the ini
              var fileExpireTime = moment(stats.mtime).add(settings.cacheMaxAge, "m").format("X");
              if (fileModTime > currentTime) {
...
*/
  ...
  readSettings().then(function(settings) {
    isCacheValid().then(function(valid) {
      if (!valid) {
